...
export async function checkRustToolchain(): Promise<boolean> {
  ...
  if (!targets.includes('riscv32imac-unknown-none-elf')) {
    const choice = await vscode.window.showWarningMessage(
      vscode.l10n.t('The RISC-V target `{0}` is not installed. Install it now?', 'riscv32imac-unknown-none-elf'),
    ...
    if (choice === vscode.l10n.t('Install')) {
      ...
      if (install.status !== 0) {
        vscode.window.showErrorMessage(vscode.l10n.t('Failed to install target; please run manually:\n rustup target add riscv32imac-unknown-none-elf'));
        return false;
      }
