...
export async function cloneXousCore(): Promise<string | undefined> {
  ...
  const destUris = await vscode.window.showOpenDialog({
    title: 'Choose a folder to clone xous-core into',
  ...
  try {
    ...
    vscode.window.showWarningMessage('Opening repo in browser. After cloning locally, select the folder in settings.');
    return;
  }
  ...
  const picked = await vscode.window.showOpenDialog({
    title: 'Select your cloned xous-core folder',
    canSelectFiles: false,
    canSelectFolders: true,
    canSelectMany: false,
