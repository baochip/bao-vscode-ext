  ...
  if (app) {
    if (!appExists(root, app)) {
      ...
      vscode.window.showErrorMessage(
        missing.length > 1
          ? vscode.l10n.t('These apps were not found under {0}: {1}', `${root}/apps-dabao`, missing.join(', '))
          : vscode.l10n.t('App "{0}" was not found under {1}.', missing[0] || app, `${root}/apps-dabao`)
      );
      return;
    }
  } else {
...
export function runBuildInTerminal(root: string, target: string, app?: string) {
  ...
  if (appArgs.length === 0) {
    ...
    term.sendText(`echo [bao] ${vscode.l10n.t('No apps specified — building target "{0}" only.', target)}`);
  } else {
...
export async function runBuildAndWait(root: string, target: string, app?: string): Promise<number> {
  ...
  if (appArgs.length === 0) {
    chan.appendLine(`[bao] ${vscode.l10n.t('No apps specified — building target "{0}" only.', target)}`);
    vscode.window.showInformationMessage(vscode.l10n.t('Building "{0}" without an app.', target));
  } else {
    vscode.window.showInformationMessage(
      vscode.l10n.t('Building "{0}" for app "{1}"…', target, appList)
  ...
  return vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: vscode.l10n.t('Baochip: Building…'), cancellable: true },
    (_progress, token) =>
