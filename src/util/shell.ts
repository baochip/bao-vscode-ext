 ...
 * shell-safe characters. POSIX wraps in single quotes (every metacharacter inert). Windows wraps
 * in double quotes with embedded " escaped; PowerShell still expands $ and backtick inside double
 * quotes, so shell-active values must be rejected before they reach a command line (see the build
 * target / app name / crate name checks in buildService).
 */
